...
public class GameController {
    ...
    @FXML private Button btnproximaonda, btnirparaloja, btnataquefisico, btnataquemagico, btnpocoes, btnhabilidade1, btnhabilidade2;
    ...
    @FXML public void acaousarpocao(ActionEvent e) {
        if(player.getpocoes().isEmpty()) { 
            textlogbatalha.appendText("voce nao tem pocoes no inventario!\n"); 
            return; 
        }
        ChoiceDialog<String> dialog = new ChoiceDialog<>(player.getpocoes().get(0), player.getpocoes());
        dialog.setTitle("usar pocao");
        dialog.setHeaderText(null);
        dialog.setContentText("escolha qual pocao usar (gasta 1 turno):");
        dialog.showAndWait().ifPresent(pocao -> {
            ...
            else if(pocao.equals("elixir da vida") || pocao.equals("coracao divino")) curarplayer(player.getvidamaxima());
            
            textlogbatalha.appendText("usou " + pocao + " e gastou o seu turno.\n");
            turnoinimigos();
            fimdeturno();
        });
    }
    ...
    private void atualizarinterface() {
        ...
        labelyen.setText("yen: " + player.getyen() + " | pontos: " + player.getpontosdemoniacos());
        ...
        btnpocoes.setText("usar pocoes (" + player.getpocoes().size() + ")");
        
        barrahpplayer.setProgress((double) Math.max(0, vidaatualplayer) / player.getvidamaxima());
        barraxp.setProgress((double) (player.getpontosdemoniacos() % 100) / 100.0);
